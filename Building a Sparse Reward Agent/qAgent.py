    ...
    def replay(self, sample_batch_size):
            ...
            for state, action, reward, next_state, done in sample_batch:
                ...
                if not done:
                    ...
                    target = reward + self.gamma * np.amax(self.brain.predict(np.reshape(next_state, [1, self.action_size ]))[0])
                target_f = self.brain.predict(np.reshape(state, [1, self.action_size ]))
                target_f[0][action] = target
                self.brain.fit(np.reshape(state,[1, self.action_size ]), target_f, epochs=1, verbose=0)
